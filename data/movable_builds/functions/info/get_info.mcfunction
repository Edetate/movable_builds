...
execute unless entity @a[tag=edta_mb_builder] if score $current_ppl_id edta_mb_ppl_id matches 1.. run tellraw @s [{"text":"This build belongs to the person with id ","color":"aqua"},{"score":{"name":"$current_ppl_id","objective":"edta_mb_ppl_id"},"color":"aqua"}, {"text":" who is currently offline.","color":"aqua"}]
