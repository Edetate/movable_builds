...
tellraw @s [{"text":"  1. Prepare your tools by toggling (drop) them to the mode you want.","color":"aqua"}]
...
tellraw @s [{"text":"  3. Put the tools in a chest, on top of a Movable Builds Trigger Command Block ","color":"aqua"}, {"text":"[Get command block]","underlined":true,"color":"dark_green","clickEvent":{"action":"run_command","value":"/function movable_builds:tools/get_command_block"}}]
